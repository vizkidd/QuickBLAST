...
// #if defined(_WIN32) && !defined(STDERR_COMMA)
...
s_HitlistReapContained(BlastHSP *hsp_array[], Int4 *hspcnt)
{
    ...
    for (iread = 1; iread < *hspcnt; iread++)
    {
        ...
        for (ireadBack = 0; ireadBack < iread && hsp1 != NULL; ireadBack++)
        {
            ...
            if (hsp2 == NULL)
            { /* hsp2 was deleted in a prior iteration. */
                continue;
            }
                ...
                hsp2->subject.frame == hsp1->subject.frame)
            {
                ...
                if (CONTAINED_IN_HSP(hsp2->query.offset, hsp2->query.end, hsp1->query.offset,
                                     hsp2->subject.offset, hsp2->subject.end,
                                     hsp1->subject.offset) &&
                    CONTAINED_IN_HSP(hsp2->query.offset, hsp2->query.end, hsp1->query.end,
                                     hsp2->subject.offset, hsp2->subject.end,
                                     hsp1->subject.end) &&
...
{
    return ((program_number == eBlastTypeRpsTblastn) ? (GET_NUCL_LENGTH(total_subj_length) - 1) / 3 : total_subj_length);
}
                 ...
                 double scale_factor)
{
    ...
    _PSICopyMatrix_double(posSearch->posFreqs,
                          ...
                          internal_pssm->ncols, internal_pssm->nrows);
    status = Kappa_impalaScaling(posSearch, compactSearch, (double)scale_factor, FALSE, sbp);
cleanup:
    ...
    for (hsp_index = 0; hsp_index < hspcnt; hsp_index++)
    {
        ...
        */
        ...
        if (new_align == NULL) /* out of memory */
            return -1;
        if (tail[frame_index] == NULL)
        { /* if the list aligns is empty; */
                         ...
                         int *align_len)
{
    ...
    while (q_pos < query_len && s_pos < subject_len)
    {
        ...
        while (q_pos < query_len && s_pos < subject_len && query_seq[q_pos] == subject_seq[s_pos])
        {
        ...
        for (n = 1; n < max_shift && q_pos + n + 1 < query_len && s_pos + n + 1 < subject_len && !match; n++)
        {

            /* mismatches */
            if (query_seq[q_pos + n] == subject_seq[s_pos + n] && query_seq[q_pos + n + 1] == subject_seq[s_pos + n + 1])
            {
            ...
            /* gap in subject */
            if (!match && query_seq[q_pos + n] == subject_seq[s_pos] && query_seq[q_pos + n + 1] == subject_seq[s_pos + 1])
            {
            ...
            /* gap in query */
            if (!match && query_seq[q_pos] == subject_seq[s_pos + n] && query_seq[q_pos + 1] == subject_seq[s_pos + n + 1])
            {
                        ...
                        int *align_len)
{
    ...
    while (q_pos >= 0 && s_pos >= 0)
    {
        ...
        /* try to skip mismatches or gaps */
        for (n = 1; n < max_shift && q_pos - n - 1 > 0 && s_pos - n - 1 > 0 && !match; n++)
        {
    ...
    *align_len += *query_ext_len > *subject_ext_len ? *query_ext_len + gaps_in_query : *subject_ext_len + gaps_in_subject;
                              ...
                              int max_shift)
{
    ...
    if (!query_seq || !query_hashes || !subject_seq || query_len < word_size || subject_len < word_size)
    {
                    ...
                    const BlastCompo_Alignment *align)
{
    ...
    /* if the whole alignment ranges where covered, return the near identical
       status */
    if (query_left_len + query_right_len >= query_len || subject_left_len + subject_right_len >= subject_len)
    {
    ...
    if (status == 0)
    {
        ...
        if (!(KAPPA_TBLASTN_NO_SEG_SEQUENCE))
        {
            if (compo_adjust_mode && (!subject_maybe_biased || *subject_maybe_biased))
            {
                ...
                if ((!shouldTestIdentical) || (shouldTestIdentical && (!s_TestNearIdentical(seqData, range->begin,
...

{
    ...
    if (!(KAPPA_BLASTP_NO_SEG_SEQUENCE))
    {
        if (compo_adjust_mode && (!subject_maybe_biased || *subject_maybe_biased))
        {
            ...
            if ((!shouldTestIdentical) || (shouldTestIdentical && (!s_TestNearIdentical(seqData, 0, queryData,
                 ...
                 const BlastExtensionParameters *extendParams)
{
         ...
         index <= queryInfo->last_context; ++index)
    {
        ...
        if ((queryInfo->contexts[index].is_valid))
        {
            near_identical_cutoff =
                (near_identical_cutoff_bits * NCBIMATH_LN2) / context->sbp->kbp_gap[index]->Lambda;
            break;
        }
    }
    ...
    {
        return Blast_RedoAlignParamsNew(&scaledMatrixInfo, &gapping_params,
 ...
 */
static BlastScoreBlk *s_BlastScoreBlk_Copy(
    ...
    Int4 number_of_contexts)
{
    ...
    if (orig->psi_matrix != NULL && orig->psi_matrix->pssm != NULL)
    {
        ...
        if (m->freqs != NULL && orig->psi_matrix->pssm->freqs != NULL)
        {
            memcpy(
                m->freqs,
        ...
        if (pm->freq_ratios != NULL && orig->psi_matrix->freq_ratios != NULL)
        {
            int i;
            for (i = 0; i < orig->psi_matrix->pssm->ncols; ++i)
            {
...
#pragma omp parallel default(none) num_threads(actual_num_threads) if (actual_num_threads > 1) \
